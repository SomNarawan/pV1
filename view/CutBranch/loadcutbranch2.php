...
include_once("../../dbConnect.php");
    ...
    ,`dim-time`.`Date`,`log-activity`.`Note`,`log-activity`.`PICs`,`log-activity`.`ID`,`dim-time`.`Date`
    FROM `log-activity` 
    ...
    WHERE `log-activity`.`isDelete` = 0 AND `dim-time`.`Year2` =  '$year' AND `db-farm`.`Name` LIKE  '$nfarm' AND`log-activity`.`DBactID`=1
