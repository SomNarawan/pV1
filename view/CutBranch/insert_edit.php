...
switch ($request) {
  ...
  case 'edit':
    ...
    $sql_edit = "UPDATE `log-activity` SET `Modify`=$t,`DIMdateID`=$dim_date,`DIMownerID`=$dim_owner,`DIMfarmID`=$dim_farm,`DIMsubFID`=$dim_subfarm,`Note`='$p_note' WHERE `ID`=$activityID";
    updateData($sql_edit);
