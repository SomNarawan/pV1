...
<?php
    ...
    //echo $sumCutBranch['sumCutBranch'];
                    ...
                    <div class="row no-gutters align-items-center">
                        <div class="col mr-2">
                            ...
                            <div class="h5 mb-0 font-weight-bold text-gray-800">
                                <?php 
                                    if($avg <=0 || $sumCutBranch['sumCutBranch'] == 0){
                                        echo 0; 
                                    }
                                    else{
                                        echo round($avg); 
...
<script>
    ...
    function loadData(year, data_search) {
        ...
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                ...
                for (i in data) {
                    if(data[i].note == null || data[i].note.trim() == ""){
                    ...
                    text += ` <tr>
                            ...
                            <td style="text-align:center;">
                                ...
                                <button type="button" id= '${i}' Pid='${data[i].ID}' class="btn btn-warning btn-sm btn-edit" data-toggle="modal" title="แก้ไข" data-target="#modal-4"><i class="fas fa-edit"></i></button>          
                                <button type="button" id='${i}' Pid='${data[i].ID}' ${styleColor} class="btn btn-primary btn-sm btn-note" data-toggle="modal" title="บันทึก"  data-target="#modal-3"><i class="far fa-sticky-note"></i></button>
                                    
                                <a href="CutBranchDetail.php?farmerID=${data[i].farmerID}&farmID=${data[i].FMID}&subfarmID=${data[i].FSID}&year=${data[i].year}&LAID=${data[i].LAID}"><button type="button" id="btn_info" class="btn btn-info btn-sm " title="รายละเอียด"><i class="fas fa-bars"></i></button></a>
                            </td>
                        </tr>`
                }
     ...
     // โหลด Photo Edit -> PICS
    function loadPhoto_Log(PICS, id) {
        let xhttp = new XMLHttpRequest();
        xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
    ...
    $(document).on('click', '.btn-edit', function() {
        ...
        //console.log(data)
        //console.log(data[id].LAID);
        loadPhoto_Log(data[id].PICs, "#p_insert_img");
