...
$sql = "SELECT `dim-user`.`Alias`,`dim-farm`.`Name`,A.`NumSubFarm`,A.`AreaRai`,A.`NumTree`,`dim-fertilizer`.`Name`,B.`Weight` AS `HarvestVol`,(SUM(`log-fertilising`.`Vol`)*2) AS `Vol1`,
    ...
    WHERE `log-fertilising`.`isDelete` = 0 AND `dim-time`.`Year2` = 2563 $text
