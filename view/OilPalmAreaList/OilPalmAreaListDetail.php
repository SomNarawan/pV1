...
$OILPALMAREALISTDETAIL = getOilPalmAreaListDetail();
?>
                        ...
                        <table class="table table-bordered table-striped table-hover table-data" width="100%">
                            ...
                            <tbody id="getData">
                                ...
                                <tr>
                                    ...
                                    <td class="text-right"><?php echo $OILPALMAREALISTDETAIL[$i]['AreaRai']; ?> ไร่
                                        <?php $OILPALMAREALISTDETAIL[$i]['AreaNgan']; ?> งาน</td>
                                    <td class="text-right"><?php echo $OILPALMAREALISTDETAIL[$i]['NumTree']; ?> ต้น</td>
                                    <td class="text-right"><?php echo $OILPALMAREALISTDETAIL[$i]['year']; ?> ปี
                                        <?php echo $OILPALMAREALISTDETAIL[$i]['month']; ?> เดือน
                                        <?php echo $OILPALMAREALISTDETAIL[$i]['day']; ?> วัน</td>
                                    <td style='text-align:center;'>
                                        <a
                                            href='./OilPalmAreaListSubDetail.php?fmid=<?= $fmid ?>&SFID=data[i].dbID&
                                        DIMfarmID=data[i].DIMfarmID&DIMSubfID=data[i].DIMSubfID&nfarm=data[i].nFarm&nsubfarm=data[i].Name&
                                        ffullname=data[i].FullName&farmer=data[i].Alias&logid=data[i].ID&numtree=data[i].NumTree;'>
                                        ...
                                        <button type='button' id='btn_delete' class="btn btn-danger btn-sm btn_edit tt"
                                            data-toggle="tooltip" title="ลบแปลง" style="margin-right:10px;"
...
$("#btn_edit_map").click(function() {
    ...
    var startLatLng = new google.maps.LatLng( < ? = $latlong[1]['Latitude'] ? > , < ? = $latlong[1][
        'Longitude'
    ] ? > );
    ...
    for (let i = 0; i < mapdetail.markers.length; i++) {
        ...
        if (i == 0) {
            ...
            marker = new google.maps.Marker({
                position: new google.maps.LatLng(mapdetail.markers[i].getPosition().lat(), mapdetail
...
var startLatLng = new google.maps.LatLng( < ? = $lat / (count($coorsfarm) - 1) ? > , < ? = $long / (count($coorsfarm) -
    1) ? > );
